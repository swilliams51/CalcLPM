...
let baseTermHelp =
    Help(title: "Base Term Start Date", instruction: "The date when the periodic rental payments commence.  If the Base Term Start Date occurs after the Funding Date then an interim term will be created and one non-periodic payment will added to the Rent Payment Schedule. To remove an interim payment set the Base Term Start Date equal to the Funding Date. For a monthly payment frequency, the Base Term Start Date cannot occur more than 90 days after the Funding Date. For all other payment frequencies the interim term cannot exceed the number of days in the Payment Frequency.")
...
let feeAmountHelp =
        Help(title: "Fee Amount", instruction: "The amount paid by the Lessor to the seller in order to purchase the Lease. Alternatively, if the Fee Type is income then the Fee represents the additional amount the seller must pay in order for the Lessor to purchase the Lease.  Do not use this field to input any Fee paid by the Lessee.  For Lessee paid fees enter an advance Rent in the Rent Screen.  Note, that if the Fee Amount is set to 0.00 then the Fee will be removed from the Investment.")
