//
//  EBOAmountTextFieldView.swift
//  CFLTax
//
//  Created by Steven Williams on 10/1/24.
//

import SwiftUI

struct EBOAmountTextFieldView: View {
    ...
    var body: some View {
        Form {
            Section (header: Text("Enter EBO Amount")) {
                eboAmountItem
            }
            .font(myFont2)
        }
        ...
        .navigationTitle("EBO Amount")
        .navigationBarTitleDisplayMode(.large)
        .navigationBarBackButtonHidden()
        .environment(\.colorScheme, isDark ? .dark : .light)
        .onAppear{
            self.amountOnEntry = myInvestment.earlyBuyout.amount
            self.myAmount = amountOnEntry
        }
    }
...
#Preview {
    EBOAmountTextFieldView(myInvestment: Investment(), path: .constant([Int]()), isDark: .constant(false), minimumEBOAmount: .constant(0.0), maximumEBOAmount: .constant(10.0))
}


extension EBOAmountTextFieldView {
    ...
    func updateForEBOAmount() {
        if myAmount.isEmpty {
            self.myAmount = "0.00"
        }
        
        if isAmountValid(strAmount: myAmount, decLow: minimumEBOAmount, decHigh: maximumEBOAmount, inclusiveLow: true, inclusiveHigh: true) == false {
            self.myAmount = self.amountOnEntry
            alertTitle = alertMaxEBO
            showAlert.toggle()
        } else {
            //Amount is Valid
            ...
            self.myInvestment.earlyBuyout.amount = myAmount
        }
...
let alertMaxEBO: String = "The calculated Lease/Loan amount exceeds the maximum allowable amount (50,000,000). As a result, the Lease/Loan will be reset to the default parameters.  It is likely that one or more of the Payment Groups has an incorrect payment amount!"
